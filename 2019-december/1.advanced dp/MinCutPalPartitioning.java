...
public class MinCutPalPartitioning {
	...
	private static void auxCuts3(String in, int cuts, MyInteger gmin) {
		if (0 == in.length()) {
			gmin.set(Math.min(gmin.get(), cuts-1));
			return;
		}
		for (int i = 0; i < in.length(); ++i) {
			if (isPalindrome(in, 0, i))
				auxCuts3(in.substring(i + 1), cuts + 1, gmin);
		}
	}
